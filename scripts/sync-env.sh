...
# CDK Outputs から .env ファイルを自動生成/更新するスクリプト
# デプロイ後に実行して環境変数を最新化
...
echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     Syncing .env from CDK Outputs          ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════╝${NC}"
echo ""
...
# CDK Outputs を取得
echo -e "${BLUE}🔍 Fetching CDK Stack Outputs...${NC}"
...
if [ $? -ne 0 ] || [ -z "$DATA_STACK_OUTPUTS" ]; then
  echo -e "${RED}❌ Failed to get DataStack outputs${NC}"
...
if [ $? -ne 0 ] || [ -z "$APP_STACK_OUTPUTS" ]; then
  echo -e "${RED}❌ Failed to get AppStack outputs${NC}"
  echo "Make sure the stack is deployed: cdk deploy MyBlog-AppStack"
  exit 1
fi

echo -e "${GREEN}✅ Successfully fetched stack outputs${NC}"
echo ""
...
# 既存の .env ファイルから保持すべき値を読み込み
if [ -f "$ENV_FILE" ]; then
  echo -e "${BLUE}📄 Reading existing .env file...${NC}"
...
# 新しい .env ファイルを生成
echo -e "${BLUE}✍️  Writing new .env file...${NC}"
...
if [ -z "$TEST_USER_PASSWORD" ] || [ "$TEST_USER_PASSWORD" == "" ]; then
  echo -e "${YELLOW}⚠️  Warning: TEST_USER_PASSWORD is not set${NC}"
